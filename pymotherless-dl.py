...
'''
    ...
    $FileInfo: pymotherless-dl.py - Last Update: 02/28/2016 Ver. 0.4.4 RC 1 - Author: cooldude2k $
'''
...
parser.add_argument("--user-agent", default="Mozilla/5.0 (Windows NT 7.0; rv:25.0) Gecko/20100101 Firefox/25.0", help="specify a custom user agent");
parser.add_argument("--referer", default="http://motherless.com/", help="specify a custom referer, use if the video access");
parser.add_argument("--proxy", default=None, help="Use the specified HTTP/HTTPS proxy");
...
if(motherless_linktype=="gallery"):
 numpages = pymotherless.get_motherless_number_of_pages(getargs.url, geturls_headers, geturls_cj);
 numcount = 1;
 while(numcount <= numpages):
  getlinks = pymotherless.get_motherless_galleries_links(getargs.url, geturls_headers, geturls_cj, page=numcount);
...
if(getargs.get_url==True):
 ...
 while(listcount < listsize):
  print(pymotherless.get_motherless_links(motherless_links[listcount], geturls_headers, geturls_cj)['url']);
...
if(getargs.get_filename==True):
 ...
 listcount = 0;
 while(listcount < listsize):
  print(pymotherless.get_motherless_links(motherless_links[listcount], geturls_headers, geturls_cj)['fullfilename']);
  listcount = listcount + 1;
...
if(getargs.get_title==True):
 ...
 listcount = 0;
 while(listcount < listsize):
  print(pymotherless.get_motherless_links(motherless_links[listcount], geturls_headers, geturls_cj)['title']);
  listcount = listcount + 1;
...
if(getargs.get_username==True):
 ...
 listcount = 0;
 while(listcount < listsize):
  print(pymotherless.get_motherless_links(motherless_links[listcount], geturls_headers, geturls_cj)['username']);
  listcount = listcount + 1;
...
if(getargs.get_views==True):
 ...
 listcount = 0;
 while(listcount < listsize):
  print(pymotherless.get_motherless_links(motherless_links[listcount], geturls_headers, geturls_cj)['numberofviews']);
  listcount = listcount + 1;
...
if(getargs.get_favorites==True):
 ...
 listcount = 0;
 while(listcount < listsize):
  print(pymotherless.get_motherless_links(motherless_links[listcount], geturls_headers, geturls_cj)['numberoffavorites']);
  listcount = listcount + 1;
...
if(getargs.get_url==False and getargs.get_pageurl==False and getargs.get_thumbnail==False and getargs.get_filename==False and getargs.get_title==False and getargs.get_username==False and getargs.get_views==False and getargs.get_favorites==False):
 ...
 while(listcount < listsize):
  if(motherless_linktype=="file"):
   pymotherless.download_motherless_links(motherless_links[listcount], geturls_headers, geturls_cj);
  if(motherless_linktype=="gallery"):
   pymotherless.download_motherless_links(motherless_links[listcount], geturls_headers, geturls_cj, outpath=os.getcwd()+os.path.sep+getargs.url.rsplit('/', 1)[-1]);
  listcount = listcount + 1;
