...
if (getlinuxdist[0].lower() == "debian" or getlinuxdist[0].lower()
        == "ubuntu" or getlinuxdist[0].lower() == "linuxmint"):
...
parser.add_argument("-d", "--distro", default=setdistroname,
                    help="enter linux distribution name")
parser.add_argument("-c", "--codename", default=setdistrocname,
                    help="enter release code name")
parser.add_argument(
    "-p",
    "--pyver",
    default=sys.version[0],
    help="enter version of python to use")
...
get_pkgbuild_dist_pre_list = [d for d in os.listdir(
    get_pkgbuild_dir) if os.path.isdir(os.path.join(get_pkgbuild_dir, d))]
...
for dists in get_pkgbuild_dist_pre_list:
    ...
    if (os.path.exists(tmp_pkgbuild_python)
            and os.path.isdir(tmp_pkgbuild_python)):
        get_pkgbuild_dist_list.append(dists)
if (getargs.distro not in get_pkgbuild_dist_list):
    print("Could not build for " + getargs.distro + " distro.")
    sys.exit()

if (getargs.distro == "debian" or getargs.distro ==
        "ubuntu" or getargs.distro == "linuxmint"):
    pypkgpath = os.path.realpath(
        getargs.source +
        os.path.sep +
        "pkgbuild" +
        os.path.sep +
        ...
        os.path.sep +
        "pydeb-gen.sh")
    pypkgenlistp = subprocess.Popen([bashlocatout,
                                     pypkgpath,
                                     getargs.source,
                                     getargs.codename],
